...
青少年心理健康问题日益严峻。据 <!-- 待补充数据：引用权威统计数据 --> 显示，我国青少年抑郁/焦虑检出率约为 <!-- 待补充数据 -->。现有心理健康服务面临三大核心挑战：

1. **资源不足**：专业心理咨询师缺口巨大，<!-- 待补充数据：咨询师/用户比例 -->
...
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────┐    │
│  │ 感知层    │  │ 评估层    │  │ 干预层    │  │ 升级层        │    │
│  │perception│→│assessment│→│intervention│→│ escalation   │    │
│  └──────────┘  └──────────┘  └──────────┘  └──────────────┘    │
│       │              │              │               │             │
│       ▼              ▼              ▼               ▼             │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │ 统一数据接口（shared/dataclasses.py）                       │   │
│  │ EmotionResult → RiskAssessment → CrisisAlert → Escalation │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                   │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                      │
│  │ 审计模块  │  │ 联邦学习  │  │ 公平性    │                      │
...
| API 层 | `api/` | `escalation.py`, `perception.py`, ... | FastAPI 接口 |
| 对比实验 | `experiments/` | `compare_*.py`, `run_all.py` | 3 组对比实验 |

### 2.3 数据流

```
用户输入 → PerceptionService.analyze_multimodal()
               │
               ▼ EmotionResult(text_emotion_probs, audio_risk_prob, confidence)
               │
         sanitize() → FeatureSummary（脱敏，差分隐私噪声）
               │
               ▼ FeatureSummary
               │
         predict() → RiskAssessment(phq9, gad7, risk_level, evidence)
               │
               ▼ RiskAssessment
               │
         state_machine.transition() → DialogState + Action
               │
               ▼
         SafetyAuditor.audit() → AuditVerdict（五轴审计）
               │
               ▼
         输出安全回复 / 触发危机升级 → escalate() → EscalationResult
```
...
基于 DistilBERT 的 5 类情绪分类模型（快乐 / 悲伤 / 焦虑 / 愤怒 / 中性）：
...
**关键创新**：非对称二元交叉熵损失（`assessment/multi_task/train.py`）

$$\mathcal{L} = -\left[\beta \cdot y \log(\hat{y}) + (1-y) \log(1-\hat{y})\right]$$
...
| 轨迹漂移 | 偏离当前对话主题 | 关键词偏离度 > 0.5 | 对话一致性要求 |
...
| 通道 | 实现方式 | 说明 |
|------|----------|------|
| Webhook | POST → mock server | 比赛 Demo 模拟，真实部署对接通知服务 |
| 站内通知 | 内存队列 | 比赛 Demo 模拟，真实部署使用 Redis Pub/Sub |
| 控制台告警 | stderr 输出 | 开发调试用 |

#### 3.6.2 咨询师排班

- 工作日 09:00-18:00：资深危机干预咨询师优先
- 周末：值班咨询师
- 夜间：仅紧急热线

#### 3.6.3 不可删除审计日志

- Append-only 约束：`delete()` / `update()` 抛出 `PermissionError`
- SHA-256 哈希链确保不可篡改
- JSONL 文件持久化
...
| 推理延迟 | 37.84 ms | 31.99 ms | **1.18× 加速** |
| 准确率下降 | - | - | **0%** |

量化方法：PyTorch 动态范围 INT8 量化（`torch.quantization.quantize_dynamic`）
...
6 项泄露验证：类型检查 / 中文泄露 / 英文泄露 / PII / UUID / 范围
    │
    ▼ 差分隐私（可选）
...
| 知情同意 | <!-- 待补充数据：同意流程设计 --> | <!-- 待补充 --> |
| 可删除权 | <!-- 待补充数据：数据生命周期管理 --> | <!-- 待补充 --> |
...
**文件**：`experiments/compare_risk_models.py`  
**数据**：1000 样本，10 维特征，80/20 划分
...
> ⭐ 高风险召回率是临床安全性的核心指标——漏诊代价远高于误诊。

### 5.3 实验 2：单模态 vs 双模态

**文件**：`experiments/compare_emotion_models.py`  
**数据**：800 样本，5 类情绪，模拟概率分布

| 模型 | Accuracy | F1 (macro) | Robustness (噪声) |
|------|----------|------------|-------------------|
...
**文件**：`experiments/compare_privacy.py`  
**数据**：1000 样本，768 维特征（模拟 DistilBERT）
...
| 指标 | 值 | 判定 |
|------|-----|------|
| Demographic Parity Diff | <!-- 待补充数据：真实数据上的值 --> | <!-- 待补充 --> |
| Equalized Odds Diff | <!-- 待补充数据：真实数据上的值 --> | <!-- 待补充 --> |
...
| 模型类型 | Acc | F1 | 说明 |
|----------|-----|-----|------|
| 联邦全局模型 | 0.6800 | 0.7265 | FedAvg 聚合 |
...
| 测试文件 | 测试数 | 状态 |
|----------|--------|------|
| test_dataclasses.py | <!-- 待补充数据 --> | ✅ |
| test_assessment.py | 22 | ✅ |
| test_state_machine.py | <!-- 待补充数据 --> | ✅ |
| test_auditor.py | 32 | ✅ |
| test_phenotype.py | 33 | ✅ |
| test_on_device.py | 12 | ✅ |
| test_sanitizer.py | 26 | ✅ |
| test_fairness.py | 19 | ✅ |
| test_federated.py | 17 | ✅ |
| test_escalation.py | 30 | ✅ |
| **总计** | **300** | **✅ 全部通过** |
...
| Python | 3.10+ | 运行环境 |
| numpy | <!-- 待补充数据 --> | 数值计算 |
| scikit-learn | <!-- 待补充数据 --> | 机器学习 |
| PyTorch | <!-- 待补充数据 --> | 模型量化 |
| transformers | <!-- 待补充数据 --> | DistilBERT |
| fairlearn | <!-- 待补充数据 --> | 公平性指标 |
| FastAPI | <!-- 待补充数据 --> | API 服务 |
| matplotlib | <!-- 待补充数据 --> | 可视化 |
